...
void displayStatistics(const string& option, const vector<Student>& LSdata, const vector<Course>& LCdata,
    const vector<Student>& PSdata, const vector<Course>& PCdata,
    const vector<Student>& ASdata, const vector<Course>& ACdata) {

if (option == "Lahore") {
calculateCampusStatistics(LSdata, LCdata);
} else if (option == "Peshawar") {
...
if (argc < 2) {
cerr << "Error: Missing argument (Lahore, Peshawar, or Combined)" << endl;
return 1;
}

string option = argv[1];  // Get the command-line argument
...
loadStudents(Studentfile_names, 16, LSdata, false);
...
loadCourses(Detailfile_names, 15, LCdata, false);
...
loadStudents(PStudentfile_names, 15, PSdata, true);
